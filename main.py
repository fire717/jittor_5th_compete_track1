...
cfg = {
    ...
    'pretrained': '/data/models/convnext_base_22k_1k_384.pkl',
...
class ImageFolder(Dataset):
    def __init__(self, root, mode, annotation_path=None, transform=None, **kwargs):
        ...
        if mode=='train':
            ...
            ###add busi
            add_data = []
            base_dir = "TrainSet/Dataset_BUSI_with_GT"
            cate = "benign"
            imgs = os.listdir(os.path.join(base_dir,cate))
            for img in imgs:
                if "_mask" in img:
                    continue
                d = [os.path.join(base_dir,cate,img), -1, 0]
                add_data.append(d)
            cate = "malignant"
            imgs = os.listdir(os.path.join(base_dir,cate))
            for img in imgs:
                if "_mask" in img:
                    continue
                d = [os.path.join(base_dir,cate,img), -1, 1]
                add_data.append(d)
            cate = "normal"
            imgs = os.listdir(os.path.join(base_dir,cate))
            for img in imgs:
                if "_mask" in img:
                    continue
                d = [os.path.join(base_dir,cate,img), 5, 2]
                add_data.append(d)
...
if __name__ == '__main__':
    ...
    if not args.testonly:
        ...
        model_paths = [
                        "./output/save/t55.pkl",
                      # "./output/save/t64_seed3407.pkl",
                      # "./output/save/t64_seed2025.pkl"
                      ]
        models = []
